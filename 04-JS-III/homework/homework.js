...
function agregarNumeros(numeros) {
  ...
  for (var i = 0 ; i < numeros.length; i++) {
    numerosTotales = numerosTotales + numeros[i];
  }
...
const empiezaConNueve = n => {
  ...
  if (n > 9 || n < -9) {
      result = n.toString()[0] === '9' ? true : false;
  } else if (n == 9 || n == -9) {
      result = true;
  } 
...
function breakStatement(numero) {
  ...
  // Tu código:   
  var guardar = [];
  var sumaBucle = numero;
  for (var i = 0; i < 10; i++) {
    sumaBucle = (sumaBucle +2);
    if (sumaBucle === i) break;
    else {
      guardar.push(sumaBucle);
    }
  }
  if (i < 10) {
